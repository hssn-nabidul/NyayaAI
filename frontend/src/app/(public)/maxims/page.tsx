      ...
      <div className="bg-ink-2/30 backdrop-blur-md p-8 md:p-12 rounded-[2.5rem] border border-white/5 space-y-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl md:text-4xl font-serif text-gold tracking-tight">Legal Maxims Library</h1>
          </div>
          <p className="text-cream/40 max-w-2xl text-sm md:text-base leading-relaxed">
            Legal maxims are the timeless principles of law. Explore Latin maxims and their 
            modern-day applications in the Indian judicial system.
          </p>
        </div>
        ...
        <form onSubmit={handleSearch} className="relative group max-w-2xl z-10">
          ...
          <input 
            ...
            placeholder="Search maxim (e.g. Nemo Judex, Caveat Emptor...)"
            className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-14 pr-32 text-cream focus:outline-none focus:border-gold/50 focus:bg-white/10 transition-all font-medium italic"
          />
          <button 
            type="submit"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {!activeMaxim ? (
            <div className="h-[400px] flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-[3rem] bg-white/2 space-y-6">
               <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center">
                 <BookMarked size={40} className="text-cream/10" />
               </div>
               <div className="text-center space-y-2">
                <p className="text-cream/20 uppercase tracking-[0.3em] text-xs font-bold">Classic Jurisprudence</p>
                <p className="text-cream/10 font-serif text-lg italic">"Lex est dictamen rationis."</p>
               </div>
            </div>
          ) : isLoading ? (
            <div className="h-[400px] flex flex-col items-center justify-center space-y-6">
              <Loader2 size={40} className="text-gold animate-spin" />
              <p className="text-cream/40 text-[10px] font-bold uppercase tracking-[0.3em] animate-pulse">Translating Wisdom...</p>
            </div>
          ) : error ? (
              ...
              <div className="bg-white/2 border border-white/5 rounded-[2.5rem] p-8 md:p-12 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 text-gold/60">
                      <Sparkles size={14} />
                      <span className="text-[10px] font-bold uppercase tracking-widest">Landmark Rulings</span>
                    </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/2 border border-white/5 rounded-3xl p-6 space-y-6 sticky top-24">
            <h3 className="text-xs font-bold uppercase tracking-widest text-gold">Famous Maxims</h3>
            <div className="space-y-2">
              {COMMON_MAXIMS.map((t) => (
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 rounded-xl text-xs text-cream/40 hover:bg-white/5 hover:text-cream/60 transition-all flex items-center justify-between group italic"
                >
                  {t}
