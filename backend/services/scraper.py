...
class NyayaScraper:
    ...
    async def search(self, form_input: str, page: int = 0) -> dict:
        """Scrape indiankanoon.org/search/"""
        ...
        for result_div in result_items:
            ...
            # Metadata is often in hlbottom (search results) or docsource_main (doc view)
            meta = result_div.select_one(".hlbottom") or result_div.select_one(".docsource_main")
            court, date_str = self._parse_meta(meta)
            
            # Fix spacing in titles (e.g. ShreyaSinghal -> Shreya Singhal)
            # Indian Kanoon sometimes has <b> tags that when stripped leave no space
            title_text = ""
    ...
    async def get_document(self, doc_id: str) -> dict:
        """Scrape indiankanoon.org/doc/{docid}/"""
        url = f"{BASE_URL}/doc/{doc_id}/"
        resp = await self._fetch_with_retry(url)
        soup = BeautifulSoup(resp.text, "lxml")
        
        # Use more robust selector for title
        title = soup.select_one(".doc_title")
        title_text = title.get_text(strip=True) if title else "Unknown"
        
        # Use more robust selector for court/meta
    ...
    async def get_citations(self, doc_id: str) -> dict:
        ...
        cites_url = f"{BASE_URL}/search/"
        cited_by_url = f"{BASE_URL}/search/"
    ...
    def _parse_meta(self, meta_tag) -> Tuple[str, str]:
        # Handle search results meta (often in hlbottom or docsource_main)
        if not meta_tag:
            return "Unknown Court", ""
        
        text = meta_tag.get_text(strip=True)
        
        # 1. Check for 'hlbottom' style (search results)
        # Example: "Supreme Court of India Cites 134-Cited by 1982-M H Beg-Full Document"
        # Example: "Constitution and Amendments Cites 0-Cited by 1900"
        if "Cites" in text or "Cited by" in text:
            # Take the part before "Cites" or "Cited by"
            court_part = text.split("Cites")[0].split("Cited by")[0].strip()
            # If the text has a date like "on 25 January, 1978", extract it
            date_match = re.search(r"on\s+(\d{1,2}\s+\w+,\s+\d{4})", text)
            date = date_match.group(1) if date_match else ""
            return court_part or "Unknown Court", date
            
        # 2. Check for standard 'docsource_main' style (doc view)
        # Example: "Supreme Court of India | 1978-01-25"
        if "|" in text:
            parts = text.split("|")
            court = parts[0].strip()
    ...
    def _extract_cited_cases(self, soup) -> List[Dict[str, str]]:
        cited = []
        # Support both /doc/ and /docfragment/ links
    ...
    def _parse_citation_results(self, soup) -> List[Dict[str, str]]:
        ...
        for div in soup.select(".result")[:20]:
            ...
            if title_tag:
                ...
                # Apply same spacing fix as in search()
                title_text = ""
                for content in title_tag.contents:
                    title_text += content.get_text(strip=True) + " "
                title_text = " ".join(title_text.split())
