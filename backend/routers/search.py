...
async def perform_search(
    ...
    """
    Public keyword search for judgments.
    """
    try:
        # Detect if it's a specific case name search (e.g., "Name vs Name")
        # 1. Normalize query: Replace "v." or " v " with " vs " for better Kanoon matching
        q_clean = q.replace(" v. ", " vs ").replace(" v ", " vs ")
        q_lower = q_clean.lower()
        
        search_query = q_clean
        if " vs " in q_lower:
            # Wrapping in quotes helps Indian Kanoon find the exact landmark case
            if not (search_query.startswith('"') and search_query.endswith('"')):
                search_query = f'"{search_query}"'

        # Convert years to Indian Kanoon date format (DD-MM-YYYY)
        from_date = f"1-1-{from_year}" if from_year else None
        to_date = f"31-12-{to_year}" if to_year else None

        # Increase pagenum internally for the first page to get more candidates for boosting
        # but keep return total/page accurate
        ...
        final_results = high_confidence_matches + other_items + constitution_items
...
async def nlp_search(
    ...
    try:
        ...
        for res in raw_results:
            ...
            if score >= 0.7:
                case_matches.append(res)
            else:
                other_items.append(res)
        
        final_results = case_matches + other_items + constitution_items
